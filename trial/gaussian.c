        ...
        for (j=0 ; j<newImageWidth ; j++) {
            for (h=i ; h<i+kHeight ; h++) {
                for (w=j ; w<j+kWidth ; w++) {
                    filtered_image[i][j][0] += gk[h-i][w-j]*image[h][w][0];
                    filtered_image[i][j][1] += gk[h-i][w-j]*image[h][w][1];
                    filtered_image[i][j][2] += gk[h-i][w-j]*image[h][w][2];
                }
            }
        }
    }
...
int main()
{
    ...
    byte *new_pixels;
    RGBQUAD *rgb = NULL;
    ReadImage("./lena/lena_2.bmp", &pixels, &width, &height,&bytesPerPixel, &xRes, &yRes, &rgb);
    ...
    applyFilter(iHeight, iWidth, image_array, kHeight, kWidth, gk, fHeight, fWidth, filtered_image);
    new_pixels = (byte*)malloc(fHeight*fWidth*3);
    arrayToPix(new_pixels, fWidth, fHeight, NULL, filtered_image);
        
    WriteImage("img_blur.bmp", new_pixels, fWidth, fHeight, 3,xRes,yRes, NULL);
